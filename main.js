import {ordenarAZ, ordenarZA, ordenarType, ordenarNum, getBaseAttackPercentage} from './data.js';
...
const showModal = (event) => {
  ...
  contentModal.innerHTML = `<span class="close">&times;</span><img src="${findCard.img}" alt=""><h1>${findCard.name}<h1><h2>${findCard.type}</h2><p>${findCard.about}</p><p>${findCard.weaknesses}</p><p>${findCard['quick-move'][0].name}</p><p>${findCard['special-attack'][0].name}</p><p>${findCard.name.toUpperCase()} has base-attack greater than ${getBaseAttackPercentage(data.pokemon, findCard)}% of Pokémons</p>`;
...
console.log(`${data.pokemon[0].name} tem o ataque superior a ${getBaseAttackPercentage(data.pokemon, data.pokemon[0])}% dos Pokem`)